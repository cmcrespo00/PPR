...
extern int id_proceso;	 // Identificador del proceso dentro de cada comunicador (coincide en ambos)
extern int size;	// N�mero de procesos que est�n resolviendo el problema
int estadoProc;	// Estado del proceso {ACTIVO, PASIVO}
        ...
        case TOKEN:
          ...
          if(estadoProc == PASIVO){
            if(id_proceso==0 && color == BLANCO && color_token == BLANCO){
              ...
              fin = true;
              MPI_Send(solucion.datos,
                       Tamanio(&solucion),
                       MPI_INT,
                       siguiente,
                       FIN,
                       comunicadorCarga);
              MPI_Recv(solucion_temporal.datos,
                       Tamanio(&solucion_temporal),
                       MPI_INT,
                       anterior,
                       FIN,
                       comunicadorCarga,
